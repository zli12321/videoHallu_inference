...
SYS_PROMPT = '''You will be given a question. Please provide a very BRIEF answer to the question.\nQuestion: {question}\nAnswer:'''
...
csv_path = './commonsense.csv'
out_file = f'./A-outputs/commonsense/qwen-VL-{model_size}B.csv'
...
for MODEL in MODELS:
    ...
    # print(generate_answer('/fs/nexus-scratch/zli12321/active-topic-modeling/SoraBench/output.mp4', 'Descripbe the video'))
    ...
    results = ['' for ele in range(len(questions))]
    start_index=0
    ...
    for i in tqdm(range(len(questions))):
        if i >= start_index:
            ...
            if type(questions[i]) == str and len(questions[i]) != 0:
                try:
                    answer = generate_answer(questions[i])
                    results[i] = answer[0]
                    print('Answer: ', answer)
                    df[col_name] = results
                    df.to_csv(out_file)
